...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* Six columns need about 40.5rem before the gaps; below that the row stacks.
     The name gets its own full-width row: the ranking bar is a percentage of
     that cell, so if the cell width varies per row - which it does when the
     status pill shares the row - a better-ranked model can draw a shorter bar.
     tools/smoke.js asserts the bars stay monotonic at 390px because of it. */
  @media (max-width: 41rem) {{
    ...
    .ix-meta {{ grid-column: 1; grid-row: 3; text-align: left; }}
    .ix-tps {{ grid-column: 2; grid-row: 3; text-align: right; }}
    .ix-eng {{ grid-column: 1; grid-row: 4; }}
  }}
